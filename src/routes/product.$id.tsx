...
useEffect(() => {
  ...
  axios
    .get(`https://bazarhub.onrender.com/products/${id}`)
...
if (!p) {
  ...
  return (
    <div className="mx-auto w-[80%] px-3 py-5 sm:px-4">
      ...
      <div className="grid gap-6 rounded-2xl border border-border bg-card p-4 shadow-sm sm:p-6 md:grid-cols-2 lg:grid-cols-[1fr_1.2fr]">
        ...
        <div>
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            ...
            <button onClick={() => toggle(p?._id)} aria-label="Wishlist"
              className="grid h-11 w-11 place-items-center rounded-lg border border-border hover:bg-muted">
              {has(p?._id) ? <FaHeart className="text-destructive" /> : <FaRegHeart />}
            </button>
          </div>
