    ...
    <div className="group relative flex flex-col overflow-hidden rounded-lg border border-border bg-card shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-lg">
      <Link to="/product/$id" params={{ id: p.id }} className="relative block aspect-square overflow-hidden bg-muted">
        ...
        <button
          ...
          aria-label="Wishlist"
        >
          {has(p.id) ? <FaHeart size={12} className="text-destructive" /> : <FaRegHeart size={12} />}
        </button>
      </Link>

      <div className="flex flex-1 flex-col gap-1.5 p-2.5">
        <Link to="/product/$id" params={{ id: p.id }} className="line-clamp-2 text-xs font-medium leading-snug text-card-foreground hover:text-primary">
          {p.name}
        </Link>
        ...
        <button
          onClick={() => { add(p); toast.success("Added to cart", { description: p.name }); }}
